...
**遇到的问题：**
- 初期对任务要求理解有偏差，误以为多步预测包含"8小时→1小时"（与单步预测重复）
- 错误地将多步预测配置为"24小时→16小时"，导致输入序列过长，模型难以学习

**解决方案：**
- 明确任务定义：单步预测=8h→1h，多步预测=8h→16h
- 删除冗余的`multistep_1h`任务配置
...
- 难以进行充分的超参数探索
- patience=30，训练轮数约200-300

**云端训练加速（智星云 A100）：**
- 租借NVIDIA A100-SXM4-40GB，仅¥2.19/小时
- 训练速度提升10-15倍，单模型训练仅需10-15分钟
- 可以使用更大batch size（512）和更高patience
- 多个模型训练epoch达到1000+

### 阶段三：超参数调优的关键发现

**问题现象：**
- 创新模型（WaveNet、HeightAttention等）多步预测性能不佳
- 初步怀疑是模型参数量过大、数据集不足
...
**解决方案：**
- 适当提高多步预测任务的学习率
- 结果：创新模型性能显著提升（WaveNet R²从~0.35提升到0.5375）

### 阶段四：创新模型设计

**发现问题：** 所有模型都未能有效利用高度信息

**HeightAttention模型创新点：**
- 将特征按高度分组，保持空间结构
- 使用多头注意力机制学习不同高度之间的依赖关系
- 引入风切变物理约束

**效果：** 多步预测任务排名第2（R²=0.5332）
...
### 1. 学习率是关键
- 学习率过低会导致模型"学不到东西"
- 不同任务（单步/多步）需要差异化的学习率设置
- 当模型表现不佳时，先检查超参数再考虑修改架构

### 2. 计算资源的重要性
- 高性能GPU（A100）相比消费级GPU（RTX 3060）可提升10-15倍训练速度
- 更多的计算资源意味着更充分的超参数探索空间
- 云GPU服务（如智星云）是高性价比的选择
